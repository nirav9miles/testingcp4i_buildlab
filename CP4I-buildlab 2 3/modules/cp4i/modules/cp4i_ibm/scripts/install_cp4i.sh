...
echo "Deploying App connect designer ${APPD_CONTENT}"
kubectl apply -n ${NAMESPACE} -f -<<EOF
${APPD_CONTENT}
EOF
...
while true; do
  if ! STATUS_LONG=$(kubectl -n ${NAMESPACE} get DesignerAuthoring des-01-quickstart-marvic --output=json | jq -c -r '.status'); then
    echo 'Error getting status'
    exit 1
  fi
